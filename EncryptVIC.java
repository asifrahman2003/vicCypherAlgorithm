 ...
 * Program Purpose: This program implements the encryption operations for the VIC (VIC InComplete) cipher.
 *   					It processes input data including an agent ID, date, phrase, anagram, and message,
 *   					performing various encoding operations.
 * 
 * 					The program reads the necessary information from a specified file, performs sanity checks,
 *   				and applies the VIC encoding algorithm to produce an encrypted message.
 * 
 *   				The VICData class holds the data necessary for the encryption process, including 
 *   				original and processed values for each input piece.
 */
...
public class EncryptVIC { 
	...
	public static VICData readVICData(String pathName, boolean isEncoded) { 
		...
		if (isEncoded) { 
			...
			try {
				...
				System.out.println("Agent ID `" + vic.agentID + "contains non-numeric characters!\n");
				System.exit(1);
			}
		}
		...
		try {
			...
			System.out.println("Date `" + vic.date + "contains non-numeric characters!\n");
			System.exit(1);
		}
		...
		if (!isEncoded) { 
			vic.message = vic.messageOriginal;
		}
		...
		if (!isEncoded) { 
			int idIndex = Character.getNumericValue(vic.date.charAt(5));
			if (vic.message.length() - idIndex < 5) {
				idIndex = vic.message.length() - 5;
			}
			vic.agentID = vic.message.substring(idIndex, idIndex + 5); 
	...
	public static void main(String[] arguements) {
		String finalResult = ""; 
		EncryptVIC.VICData readData = EncryptVIC.readVICData(arguements[0], true); 
		...
		while (index < readData.message.length()) {
			finalResult += boardKey.get(readData.message.charAt(index) - 65);
			index++; // Increment index
		}

		if (finalResult.length() <= lastDigit) {
			finalResult += readData.agentID;
		} else {
			String substring1 = finalResult.substring(0, lastDigit);
			String substring2 = finalResult.substring(lastDigit);
			finalResult = substring1 + readData.agentID + substring2;
		}
